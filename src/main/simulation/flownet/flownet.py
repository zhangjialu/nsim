...
for i in range(inner_nodes_length):
    for j in range(inner_nodes_length):
        # if i == j:
        #     A[i][j] = np.sum(D**2/(R*np.abs(G))) + np.sum(DE**2/(RE*np.abs(GE))) + 0.001
        # else:
        ...
        if i == j:
            A[i][j] = 1 + 0.2/1e-8*(np.sum(D/(2*R*G)) + np.sum(DE/(2*RE*GE)))
        else:
            A[i][j] = -D[i][j]*0.2/(2*1e-8*R[i][j]*G[i][j])
...
for i in range(inner_nodes_length):
    ...
    boundary = 0
    for j in range(inner_nodes_length):
        inner += D[i][j]*(H[i][j] + G[i][j]**2)/(2*R[i][j]*G[i][j])
    for k in range(boundary_nodes_length):
        boundary += DE[i][k]*(PE[k] + GE[i][k]**2)/(2*RE[i][k]*GE[i][k])
    # for j in range(inner_nodes_length):
    ...
    B[i] = P[i] + 0.2/1e-8*(inner + boundary)
...
for i in range(inner_nodes_length):
    for j in range(inner_nodes_length):
        Gx[i][j] = np.sqrt(np.abs(D[i][j]*(Px[j] - Px[i])/R[i][j]))


GEx = np.zeros((inner_nodes_length, boundary_nodes_length))
for i in range(inner_nodes_length):
    for k in range(boundary_nodes_length):
        GEx[i][k] = np.sqrt(np.abs(DE[i][k]*(PE[k] - Px[i])/RE[i][k]))
    ...
    for j in range(inner_nodes_length):
        if i >= j:
            inner_device_id = inner_path.get(
                (inner_nodes_list[i].id, inner_nodes_list[j].id))
            if inner_device_id:
                result[inner_device_id] = {'Qm': Gx[i][j]}
...
for i in range(inner_nodes_length):
    for j in range(boundary_nodes_length):
        ...
        if boundary_device_id:
            result[boundary_device_id] = {'Qm': GEx[i][j]}
